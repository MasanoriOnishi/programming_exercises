...
describe('DOMSource.events()', function () {
    ...
    it('should setup click detection with events() after run() occurs', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.h3('.test2.myelementclass', 'Foobar')),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget()),
    ...
    it('should setup click detection on a ready DOM element (e.g. from server)', function (done) {
        ...
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(containerElement),
        }), sinks = _a.sinks, sources = _a.sources, run = _a.run;
        var dispose = run();
        sources.DOM.select('.myelementclass').events('click').addListener({
            next: function (ev) {
    ...
    it('should catch events using id of root element in DOM.select', function (done) {
        ...
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget('parent-001')),
        }), sinks = _a.sinks, sources = _a.sources, run = _a.run;
        var dispose;
        // Make assertions
    ...
    it('should catch events using id of top element in DOM.select', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.h3('#myElementId', 'Foobar')),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget('parent-002')),
        }), sinks = _a.sinks, sources = _a.sources, run = _a.run;
        var dispose;
        // Make assertions
    ...
    it('should catch interaction events without prior select()', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.parent', [lib_1.h3('.myelementclass', 'Foobar')])),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget()),
        ...
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.h3('.top-most', [
                    lib_1.h2('.bar', 'Wrong'),
                    lib_1.div('.foo', [lib_1.h4('.bar', 'Correct')]),
                ])),
            };
        }
        var _a = run_1.setup(app, {
    ...
    it('should catch events from many elements using DOM.select().events()', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.parent', [
                    lib_1.h4('.clickable.first', 'First'),
                    lib_1.h4('.clickable.second', 'Second'),
                ])),
            };
        }
        var _a = run_1.setup(app, {
    ...
    it('should catch interaction events from future elements', function (done) {
        function app(sources) {
            return {
                DOM: concat_1.default(xstream_1.default.of(lib_1.h2('.blesh', 'Blesh')), xstream_1.default.of(lib_1.h3('.blish', 'Blish')).compose(delay_1.default(150)), xstream_1.default.of(lib_1.h4('.blosh', 'Blosh')).compose(delay_1.default(150))),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget('parent-002')),
    ...
    it('should catch bubbling events in a DocumentFragment', function (done) {
        ...
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div([lib_1.div('.clickable', 'Hello')])),
            };
        }
        if (!thisBrowserBubblesFragmentEvents) {
            ...
            var _a = run_1.setup(app, {
                DOM: lib_1.makeDOMDriver(renderTarget),
    ...
    it('should have currentTarget or ownerTarget pointed to the selected parent', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.top', [lib_1.h2('.parent', [lib_1.span('.child', 'Hello world')])])),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget()),
    ...
    it('should catch a non-bubbling Form `reset` event', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.parent', [lib_1.form('.form', [lib_1.input('.field', { type: 'text' })])])),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget()),
    ...
    it('should catch a non-bubbling click event with useCapture', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.parent', [lib_1.div('.clickable', 'Hello')])),
            };
        }
        function click(el) {
    ...
    it('should catch a blur event with useCapture', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.parent', [
                    lib_1.input('.correct', { type: 'text' }, []),
                    lib_1.input('.wrong', { type: 'text' }, []),
                    lib_1.input('.dummy', { type: 'text' }),
                ])),
            };
        }
    ...
    it('should allow restarting of event streams from isolated components', function (done) {
        ...
        function component(sources) {
            ...
            var itemMouseClick$ = itemMouseDown$
                ...
                .flatten();
            switchSubject.map(function () { return itemMouseClick$; }).flatten().addListener({
                next: function (ev) {
                    outSubject.shamefullySendNext(ev);
                },
            });
            return {
                DOM: xstream_1.default.of(lib_1.button('.item', ['stuff'])),
            };
        }
        function app(sources) {
    ...
    it('should allow preventing default event behavior', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.parent', [lib_1.button('.button')])),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget()),
