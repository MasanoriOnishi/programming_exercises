        ...
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.h3('.top-most', [
                    lib_1.h2('.bar', 'Wrong'),
                    lib_1.div({ isolate: 'foo' }, [lib_1.h4('.bar', 'Correct')]),
                ])),
            };
        }
...
describe('isolation', function () {
    it('should prevent parent from DOM.selecting() inside the isolation', function (done) {
        function app(sources) {
            var child$ = sources.DOM.isolateSink(xstream_1.default.of(lib_1.div('.foo', [lib_1.h4('.bar', 'Wrong')])), 'ISOLATION');
            var vdom$ = xstream_1.default
                .combine(xstream_1.default.of(null), child$)
                .map(function (_a) {
                var _ = _a[0], child = _a[1];
                return lib_1.h3('.top-most', [child, lib_1.h2('.bar', 'Correct')]);
            });
            return {
                DOM: vdom$,
            };
    ...
    it('should not occur with scope ":root"', function (done) {
        function app(sources) {
            var child$ = sources.DOM.isolateSink(xstream_1.default.of(lib_1.div('.foo', [lib_1.h4('.bar', 'Not wrong')])), ':root');
            var vdom$ = xstream_1.default
                .combine(xstream_1.default.of(null), child$)
                .map(function (_a) {
                var _ = _a[0], child = _a[1];
    ...
    it('should apply only between siblings when given scope ".foo"', function (done) {
        function app(sources) {
            var foo$ = sources.DOM.isolateSink(xstream_1.default.of(lib_1.div('.container', [lib_1.h4('.header', 'Correct')])), '.foo');
            var bar$ = sources.DOM.isolateSink(xstream_1.default.of(lib_1.div('.container', [lib_1.h3('.header', 'Wrong')])), '.bar');
            var vdom$ = xstream_1.default
                .combine(foo$, bar$)
                .map(function (_a) {
                var foo = _a[0], bar = _a[1];
                return lib_1.div('.top-most', [foo, bar, lib_1.h2('.header', 'Correct')]);
            });
            return {
                DOM: vdom$,
            };
    ...
    it('should work with thunks', function (done) {
        function app(sources) {
            var child$ = sources.DOM.isolateSink(xstream_1.default.of(lib_1.thunk('div.foo', function () { return lib_1.div('.foo', [lib_1.h4('.bar', 'Wrong')]); }, [])), 'ISOLATION');
            var vdom$ = xstream_1.default
                .combine(xstream_1.default.of(null), child$)
                .map(function (_a) {
                var _ = _a[0], child = _a[1];
    ...
    it('should allow using elements() in an isolated main() fn', function (done) {
        function main(sources) {
            ...
            var vnode$ = elem$.map(function (elem) { return lib_1.h('div.bar', 'left=' + elem.offsetLeft); });
            return {
                DOM: vnode$,
            };
        }
        ...
        sources.DOM.select(':root').elements().drop(1).take(1).addListener({
            next: function (rootElement) {
                var barElem = rootElement.querySelector('.bar');
    ...
    it('should allow parent to DOM.select() in its own isolation island', function (done) {
        function app(sources) {
            ...
            var child$ = sources.DOM.isolateSink(islandVDom$.map(function (islandVDom) {
                return lib_1.div('.foo', [islandVDom, lib_1.h4('.bar', 'Wrong')]);
            }), 'ISOLATION');
            var vdom$ = child$.map(function (child) { return lib_1.h3('.top-most', [child]); });
            return {
                DOM: vdom$,
    ...
    it('should isolate DOM.select between parent and (wrapper) child', function (done) {
        function Frame(sources) {
            ...
            var vdom$ = sources.content$.map(function (content) {
                return lib_1.h4('.foo.frame', { style: { backgroundColor: 'lightblue' } }, [content]);
            });
            return {
                DOM: vdom$,
                click$: click$,
    ...
    it('should allow a child component to DOM.select() its own root', function (done) {
        function app(sources) {
            var child$ = sources.DOM.isolateSink(xstream_1.default.of(lib_1.span('.foo', [lib_1.h4('.bar', 'Wrong')])), 'ISOLATION');
            return {
                DOM: child$.map(function (child) { return lib_1.h3('.top-most', [child]); }),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget()),
    ...
    it('should allow DOM.selecting svg elements', function (done) {
        function App(sources) {
            ...
            var svgTriangle = lib_1.svg({ attrs: { width: 150, height: 150 } }, [
                lib_1.svg.polygon({
                    attrs: {
                        class: 'triangle',
    ...
    it('should allow DOM.select()ing its own root without classname or id', function (done) {
        function app(sources) {
            var child$ = sources.DOM.isolateSink(xstream_1.default.of(lib_1.span([lib_1.h4('.bar', 'Wrong')])), 'ISOLATION');
            return {
                DOM: child$.map(function (child) { return lib_1.h3('.top-most', [child]); }),
            };
        }
    ...
    it('should allow DOM.select()ing all elements with `*`', function (done) {
        function app(sources) {
            var child$ = sources.DOM.isolateSink(xstream_1.default.of(lib_1.span([lib_1.div([lib_1.h4('.foo', 'hello'), lib_1.h4('.bar', 'world')])])), 'ISOLATION');
            return {
                DOM: child$.map(function (child) { return lib_1.h3('.top-most', [child]); }),
            };
        }
    ...
    it('should allow isolatedDOMSource.events() to work without crashing', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.h3('.top-most', [lib_1.div({ isolate: 'foo' }, [lib_1.h4('.bar', 'Hello')])])),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget()),
                ...
                DOM: childExistence$.map(function (exists) {
                    return lib_1.div([
                        lib_1.div('.top-most', { isolate: 'top' }, [
                            lib_1.h2('.bar', 'Wrong'),
                            exists ? lib_1.div({ isolate: 'foo' }, [lib_1.h4('.bar', 'Correct')]) : null,
                        ]),
                    ]);
                }),
            };
        ...
        function Child(sources) {
            return {
                DOM: sources.DOM
                    ...
                    .mapTo(1)
                    .startWith(0)
                    .map(function (num) { return lib_1.div('.container', [lib_1.h3('.foo', 'Child foo')]); }),
            };
        }
        function main(sources) {
            ...
            var vnode$ = xstream_1.default
                ...
                return xstream_1.default
                    .combine.apply(xstream_1.default, children.map(function (child) { return child.DOM; })).map(function (childVNodes) { return lib_1.div('.parent', childVNodes); });
            })
                .flatten();
            return {
                DOM: vnode$,
    ...
    it('should handle events when child is removed and re-added', function (done) {
        ...
        function main(sources) {
            ...
            var innerDOM$ = xstream_1.default
                ...
                .startWith(0)
                .map(function (x) { return (x === 1 ? xstream_1.default.of(lib_1.div()) : child.DOM); })
                .flatten();
            return {
                DOM: innerDOM$,
            };
    ...
    it('should allow an isolated child to receive events when it is used as ' +
        ...
        function Component(sources) {
            ...
            return {
                DOM: xstream_1.default.of(lib_1.div('.component', {}, [lib_1.button('.btn', {}, 'Hello')])),
            };
        }
        function main(sources) {
    ...
    it('should allow null or undefined isolated child DOM', function (done) {
        function child(sources) {
            ...
            var vdom$ = visible$.map(function (visible) { return (visible ? lib_1.h4('child') : null); });
            return {
                DOM: vdom$,
            };
        }
        function main(sources) {
            ...
            var vdom$ = childSinks.DOM.map(function (childVDom) {
                return lib_1.div('.parent', [childVDom, lib_1.h2('part of parent')]);
            });
            return {
                DOM: vdom$,
            };
    ...
    it('should not lose event delegators when components are moved around', function (done) {
        function component(sources) {
            var click$ = sources.DOM
                ...
                .mapTo('clicked');
            return {
                DOM: xstream_1.default.of(lib_1.button('.click-me', 'click me')),
                click$: click$,
            };
        }
        function app(sources) {
            ...
            return {
                DOM: xstream_1.default.combine(position$, comp.DOM).map(function (_a) {
                    ...
                    var children = position === '1'
                        ? [lib_1.div([childDom]), lib_1.div()]
                        : [lib_1.div(), lib_1.div([childDom])];
                    return lib_1.div(children);
                }),
                click$: comp.click$,
            };
        }
