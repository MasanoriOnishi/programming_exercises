...
describe('DOM Rendering', function () {
    it('should render DOM elements even when DOMSource is not utilized', function (done) {
        function main() {
            return {
                DOM: xstream_1.default.of(lib_1.div('.my-render-only-container', [
                    lib_1.h2('Cycle.js framework'),
                ])),
            };
        }
        run_1.run(main, {
    ...
    it('should support snabbdom dataset module by default', function (done) {
        ...
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.my-class', {
                    dataset: { foo: 'Foo' },
                })),
            };
        }
    ...
    it('should render in a DocumentFragment as container', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.select('.my-class', [
                    lib_1.option({ attrs: { value: 'foo' } }, 'Foo'),
                    lib_1.option({ attrs: { value: 'bar' } }, 'Bar'),
                    lib_1.option({ attrs: { value: 'baz' } }, 'Baz'),
                ])),
            };
        }
        var docfrag = document.createDocumentFragment();
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(docfrag),
    ...
    it('should reuse existing DOM tree under the given root element', function (done) {
        ...
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(rootElem),
        }), sinks = _a.sinks, sources = _a.sources, run = _a.run;
        var dispose;
        sources.DOM.select(':root').elements().drop(1).take(1).addListener({
            next: function (root) {
    ...
    it('should give elements as a value-over-time', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.merge(xstream_1.default.of(lib_1.h2('.value-over-time', 'Hello test')), xstream_1.default.never()),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget()),
    ...
    it('should allow snabbdom Thunks in the VTree', function (done) {
        function renderThunk(greeting) {
            return lib_1.h4('Constantly ' + greeting);
        }
        // The Cycle.js app
        function app(sources) {
            return {
                DOM: xstream_1.default.periodic(10).take(5).map(function (i) {
                    return lib_1.div([
                        lib_1.thunk('h4', 'key1', renderThunk, ['hello' + 0]),
                    ]);
                }),
            };
        }
    ...
    it('should render embedded HTML within SVG <foreignObject>', function (done) {
        ...
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.svg({ attrs: { width: 150, height: 50 } }, [
                    lib_1.svg.foreignObject({ attrs: { width: '100%', height: '100%' } }, [
                        lib_1.p('.embedded-text', 'This is HTML embedded in SVG'),
                    ]),
                ])),
            };
        }
    ...
    it('should render textContent "0" given hyperscript content value number 0', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.my-class', 0)),
            };
        }
        var _a = run_1.setup(app, {
            DOM: lib_1.makeDOMDriver(createRenderTarget()),
